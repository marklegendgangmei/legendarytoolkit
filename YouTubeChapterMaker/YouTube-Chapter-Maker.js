// ============================================================
// YouTube Chapter Maker - Safe & Simple
// No API keys required. Paste description to extract chapters.
// ============================================================
    ...
    function buildApp(container) {
        ...
        glassCard.innerHTML = `
            <div class="d-flex flex-column gap-3">
                <div class="text-center text-md-start">
                    ...
                    <p class="text-secondary">Paste a video description to extract chapters, or add them manually.</p>
                </div>

                <!-- Video URL + Thumbnail (oEmbed, no key) -->
                <div class="row g-3">
                    <div class="col-12">
                        <label class="form-label fw-semibold"><i class="bi bi-link-45deg"></i> YouTube URL (optional)</label>
                        <div class="input-group">
                            <input type="text" class="form-control" id="chapter-maker-url" placeholder="https://youtu.be/... or video ID">
                            <button class="btn btn-dark-custom btn-modern" id="chapter-maker-fetch-btn"><i class="bi bi-image"></i> Load Thumbnail & Title</button>
                        </div>
                        <div class="form-text">Only used to display video info – no description is fetched (privacy safe).</div>
                    </div>
                </div>

                <!-- Video info preview (oEmbed) -->
                <div id="video-info-panel" style="display: none;">
                    <div class="d-flex gap-3 align-items-center p-3 bg-light rounded-4">
                        ...
                        <div>
                            ...
                            <a id="video-watch-link" href="#" target="_blank">Watch on YouTube <i class="bi bi-box-arrow-up-right"></i></a>
                        </div>
                    </div>
                </div>

                <!-- Paste description to extract chapters -->
                <div class="bg-light bg-opacity-50 p-3 rounded-4">
                    <label class="fw-semibold"><i class="bi bi-file-text"></i> 1. Copy video description from YouTube</label>
                    <textarea id="description-textarea" rows="4" class="form-control mt-1" placeholder="Paste the video description here...&#10;Example:&#10;0:00 Introduction&#10;2:15 Main content&#10;5:30 Conclusion"></textarea>
                    <button id="extract-chapters-btn" class="btn btn-dark-custom btn-modern mt-2"><i class="bi bi-magic"></i> 2. Extract Chapters from Description</button>
                    <div class="form-text mt-2">The tool finds timestamps like <code>0:00 Title</code> or <code>1:23:45 Topic</code> at the start of lines.</div>
                </div>
        ...
        function renderChapterList() {
            ...
            if (chaptersArray.length === 0) {
                listDiv.innerHTML = '<div class="text-muted text-center py-4">No chapters yet. Paste a description and click "Extract".</div>';
                return;
            }
        ...
        // Extract chapters from pasted description (line by line)
        function extractChaptersFromText(description) {
            if (!description || !description.trim()) {
                showMessage("Please paste a description first.", "warning");
                return false;
            }
            ...
            if (newChapters.length === 0) {
                showMessage("No timestamps found. Make sure each chapter starts a line with e.g., '1:30 Title'.", "danger");
                return false;
            }
            ...
            else showMessage(`Added ${added} chapter(s) from description.`);
            renderChapterList();
            updateOutput();
            return added > 0;
        }
        ...
        function manualExtract() {
            ...
            extractChaptersFromText(description);
        }

        // Fetch video info via oEmbed (no API key)
        async function fetchVideoInfo() {
            ...
            try {
                ...
                showMessage(`Loaded: ${data.title}`, "success");
            } catch (err) {
                showMessage("Could not load video info. Check the URL.", "danger");
            } finally {
        ...
        // Bind events
        document.getElementById('chapter-maker-fetch-btn').addEventListener('click', fetchVideoInfo);
